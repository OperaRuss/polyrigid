...
def confusionMatrix(targetSeg, predictionSeg, outPath,label,prefix:str=""):
    ...
    out = {'target':label,'TP':TP,'FP':FP,'FN':FN,'TN':TN,'DICE':2*(TP/IoU),
           'accuracy':(TP+TN)/(TP+FP+FN+TN)*100,'precision':TP/(TP+FP)*100,
           'recall':TP/(TP+FN)*100}

    return out

def sequenceConfusion(outPath,params:str=""):
    fpResults = "../images/results" # inPath
...
def _getStaticMeshes(fpInPath, params:str="",filePrefix:str='warped_seg_', fpOutPath:str='../images/results/meshes'):
    '''
    ...
    if not os.path.exists(fpOutPath):
        os.mkdir(fpOutPath)
    if not os.path.exists(os.path.join(fpOutPath,params)):
        os.mkdir(os.path.join(fpOutPath,params))
    ...
    for file in sorted(glob.glob(fpInPath+"/"+filePrefix+"*.nii")):
        ...
        outFile = params + '_' + basename + ".vtk"
        writer = vtk.vtkPolyDataWriter()
        writer.SetInputData(poly)
        writer.SetFileName(os.path.join(os.path.join(fpOutPath,params),outFile))
        writer.Update()
...
def getStaticMeshes(fpResults:str='../images/results',params:str="",filePrefix:str='warped_seg_'):
    ...
    for file in sorted(glob.glob(fpResults + "/frame_*_to_frame_*")):
        if os.path.isdir(file):
            ...
            _getStaticMeshes(file,params,filePrefix,os.path.join(fpOut,subdir))

def displayFrames(params:str=""):
    plt.close('all')

    fpResults = "../images/results" # inDir
    ...
    for file in sorted(glob.glob(fpResults + "/frame_*_to_frame_*")):
        if os.path.isdir(file):
            fSource = os.path.basename(file).split('_')[1]
            fTarget = os.path.basename(file).split('_')[-1]
